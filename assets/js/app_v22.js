...
(function() {
    ...
    function init() {
        ...
        if (!uploadZone || !formCarga) return; // No estamos en la pÃ¡gina de carga
    ...
    function procesarArchivoSeleccionado(file) {
        ...
        if (extensionesValidas.indexOf(extension) === -1) {
            mostrarAlerta('danger', 'Formato no vÃ¡lido. Solo se aceptan archivos .xlsx o .csv');
            return;
        }
        
        if (file.size > 5 * 1024 * 1024) {
            mostrarAlerta('danger', 'El archivo excede el tamaÃ±o mÃ¡ximo de 5MB');
            return;
        }
        ...
        document.getElementById('infoArchivo').textContent = 
            'TamaÃ±o: ' + formatBytes(file.size) + ' | Tipo: ' + extension.toUpperCase();
    ...
    function setupFormSubmit() {
        formCarga.addEventListener('submit', function(e) {
            ...
            xhr.onerror = function() {
                ocultarModalCargando();
                mostrarAlerta('danger', 'Error de conexiÃ³n. Verifique su conexiÃ³n a internet.');
            };
    ...
    function mostrarPreview(response) {
        ...
        if (!seccionPreview || !resumenCarga) {
            console.error('Elementos DOM críticos no encontrados');
            mostrarAlerta('danger', 'Error: elementos de la página no encontrados. Recargue la página.');
            return;
        }
    ...
    function poblarTablaPreview(registros) {
        ...
        for (var i = 0; i < registros.length; i++) {
            ...
            if (reg.estado === 'valido') {
                ...
                estadoBadge = '<span class="badge bg-success"><i class="bi bi-check-circle"></i> VÃ¡lido</span>';
            } else if (reg.estado === 'codigo_invalido') {
                rowClass = 'table-warning';
                estadoBadge = '<span class="badge bg-warning text-dark"><i class="bi bi-exclamation-triangle"></i> CÃ³digo</span>';
            } else {
    ...
    function setupButtons() {
        document.getElementById('btnCancelar').addEventListener('click', function() {
            if (!confirm('Â¿EstÃ¡ seguro de cancelar? Se perderÃ¡n los datos precargados.')) {
                return;
            }
    ...
    function confirmarCarga() {
        ...
        xhr.onload = function() {
            ...
            if (xhr.status === 200) {
                ...
                mostrarAlerta('danger', 'Error del servidor (cÃ³digo: ' + xhr.status + ')');
            }
        };
        
        xhr.onerror = function() {
            ocultarModalCargando();
            mostrarAlerta('danger', 'Error de conexiÃ³n.');
        };
    ...
    function mostrarResultadoFinal(response) {
        ...
        if (response.success) {
            html = '<div class="resultado-exito">' +
                ...
                '<h3>Â¡Carga Completada Exitosamente!</h3>' +
                '<p class="lead">Se han insertado <strong>' + response.insertados + '</strong> diploma(s) en el sistema.</p>' +
                '<p class="text-muted">Los registros ya estÃ¡n disponibles para validaciÃ³n.</p>' +
            '</div>';
            
            // Si hay registros no cargados, mostrar opciÃ³n de descarga
    ...
    // ============================================
    function nuevaCarga() {
        // Recargar pÃ¡gina para resetear completamente
    ...
    function ocultarAlerta() {
        document.getElementById('alertaGlobal').innerHTML = '';
    }
